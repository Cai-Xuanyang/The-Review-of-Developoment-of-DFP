...
\begin{equation}
    ...
    \label{eq:dfp_update}
\end{equation}
where $\dd_k = -\HH_k \ggg_k$ is the search direction, set $\ggg_k = \nabla f(\xx_k)$and $\yy_k = \ggg_{k+1} - \ggg_k$.
...
that $\Delta \phi_k = \phi(\HH_{k+1}) - \phi(\HH_k)$ after one DFP update. Let us denote $\tau_k = \yy_k^\trans \dd_k$ and $\delta_k = \yy_k^\trans \HH_k \yy_k$. Using the DFP update formula \eqref{eq:dfp_update}, one can derive exact expressions for the trace and determinant updates:
\begin{align}
...
\begin{equation}
    \Delta \phi_k \le C_1 - C_2 \dfrac{\|\ggg_k\|^2}{\ggg_k^trans \HH_k \ggg_k}.
